...
>
> 本文按「原理 → 推导 → 利用 → 修复 → 延伸」组织的，每一节的 payload 都经过实测。
> 建议先自己在靶场里试一遍再看答案。
...
> 为什么用 `-1` 而不是 `1`？
> 因为原查询会返回一行真实数据，UNION 的结果会追加在后面。
> 用不存在的 id（如 `-1`）让原查询返回空，页面就只显示我们的注入结果，更清晰。

### 2.5 第四步：拖出目标数据

目标：读取 `users.secret`。注意原语句的列是
`id, username, email, role`，我们要把 `secret` 塞进其中某一列的位置：
...
—— 过滤器把关键字「还原」成了我们的 payload。这直观展示了黑名单的荒谬之处。
...
> 「我用 `addslashes()` / `mysqli_real_escape_string()` 转义了引号，所以安全。」
...
❌ 不要依赖黑名单过滤关键字
❌ 不要依赖 addslashes / real_escape_string 作为唯一防线
❌ 不要以为「前端已经校验过了」就安全（请求可以被直接构造）
❌ 不要把数据库错误原样返回给客户端
```
