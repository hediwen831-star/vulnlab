...
把类定义抠出来，本地生成 payload 再打过去。手工拼串反而容易在长度上翻车。
...
是在命令行里单独 `unserialize` 一遍、打印 `getcwd()` 之后得出的。
...
这个细节本身就是一个结论：反序列化的危害发生在「以为一切都已经结束」之后。

这个坑是双向的：写检测逻辑的人如果不知道 `__destruct` 的时机，
会写出「永远显示没有副作用」的检查，然后误以为漏洞不存在。
反过来，防守方如果只在业务逻辑里加校验，也不会拦住它 ——
因为触发点在业务逻辑之外。
